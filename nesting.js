...
employeeUpdater=()=>{
  for(i of employees){
    if (i.firstName==="Lorie"){
      i.department="HR" 
  }
    else if(i.firstName==="Theo"){
...
//Code Here
removeDuplicates= ()=>{
  return Array.from(new Set(workplaceAccidents))
}
...
looper = ()=>{
  for(arr in numsArr){
    console.log(arr)
    for (i=0; i<numsArr[arr].length; i++){
      numsArr[arr][i]%2===0?numsArr[arr][i]="even":numsArr[arr][i]="odd"
    }
  }
  return numsArr
}
